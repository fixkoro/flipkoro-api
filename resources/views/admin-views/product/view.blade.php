                ...
                <div>
                    <div class="media flex-nowrap flex-column flex-sm-row gap-3 flex-grow-1">
                        <div class="d-flex flex-column align-items-center __min-w-165px">
                            ...
                            @if ($productActive)
                                @php($reactData = getWebConfig(name:'react_setup'))
                                @php($reactStatus = isset($reactData['status']) ? $reactData['status'] : 0)
                                @php($reactDomain = isset($reactData['react_domain']) ? $reactData['react_domain'] : null)
                                @php($productUrl = ($reactStatus && !empty($reactDomain)) ? $reactDomain.'/products/'.$product['slug'] : route('product', $product['slug']))
                                <a href="{{ $productUrl }}"
